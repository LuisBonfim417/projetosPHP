...
class CrossTableWebReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("tb_setpoing" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_setpoint_email" == $this->xml_array['tables'][0])
			$isDB = true;
		if("aircontrolweb_users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_monitoramento" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_reference" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_setpoint_bloqueia" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_setpoint_sms" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_status" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_ticket" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_controle" == $this->xml_array['tables'][0])
			$isDB = true;
		if("tb_agendamento" == $this->xml_array['tables'][0])
			$isDB = true;
		if("24horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("12horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("24horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("aircontrolweb_dyperugrights" == $this->xml_array['tables'][0])
			$isDB = true;
		if("aircontrolweb_users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("aircontrolweb_users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("9horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("6horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("3horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("12horas" == $this->xml_array['tables'][0])
			$isDB = true;
		if("hoje" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_analise_temp_maximo_acima" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_analise_temp_minima_abaixo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_monitoramento_por_periodo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_monitoramento_por_periodo_leitura_max_min_med" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_monitoramento_por_periodo_leitura_maxima" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_monitoramento_por_periodo_leitura_minima" == $this->xml_array['tables'][0])
			$isDB = true;
		if("vwv_tag_user" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
