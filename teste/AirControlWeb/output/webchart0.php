...
$b_includes .= alertDialog();
	...
	$b_includes.='$("#radio_db,#radio_project,#radio_custom").click(function(){
		...
		if($(this).attr("id")=="radio_db")
		{
			...
			li_selected="db";
			$("#tl").html("'."Selecione a tabela que usará para criar o gráfico:".'");
	...
	$b_includes .= '
			...
			$("#tl").html("'."Selecione a consulta SQL que usará para criar o gráfico:".'");
			NEXT_PAGE_URL = "'.GetTableLink("webchart3").'";
			PREV_PAGE_URL = "'.GetTableLink("webreport").'";';
	$b_includes .= '
			$("td[id=row1], td[id=row2]").hide();'."\r\n";
...
if(postvalue("sqlname"))
	$b_includes .= '
					...
					$("#radio_custom").click();
					li_selected="custom";
					$("#tl").html("'."Selecione a consulta SQL que você usará para criar o relatório".'");
					';
else
{	
	if(is_wr_db() && count($arr_tables_db))
