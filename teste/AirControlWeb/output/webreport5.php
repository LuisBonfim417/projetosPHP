...
if (@$_SESSION['webreports']['tables'][0] != "") {
	...
	$xt->assign("report_table",", "."Tabela".": ".$stable);
} else {
	...
	$("#nextbtn, #backbtn, td[id^=row], #savebtn, #saveasbtn, #previewbtn").click(function(){
		...
		if( this.id == "savebtn" || this.id == "previewbtn") {
			...
			$.ajax({
				...
				error: function() {
					$("#alert").html("<p>'."Algum problema ocorreu enquanto salvava".'</p>").dialog("open");
				}
			});
		}
		thisid=this.id;
...
if(count($_SESSION['webreports']['group_fields'])>1)
{
	...
	<tr>
		<td class="tdimg rl" style="text-align:center">
			<img src="'.GetRootPathForResources("images/layout_stepped.png").'" id="stepped" border="0" alt="'."Layout Intercalada".'" />
			<img src="'.GetRootPathForResources("images/layout_block.png").'" id="block" border="0" alt="'."Layout de Bloco".'" />
		</td>
	</tr>
	<tr>
		<td class="tdimg rl" style="text-align:center">
		  <img src="'.GetRootPathForResources("images/layout_align.png").'" id="align" border="0" alt="'."Layout de Alinhamento".'" />
		  <img src="'.GetRootPathForResources("images/layout_outline.png").'" id="outline" border="0" alt="'."Layout de Contorno".'" />
		</td>
	</tr>';
}
else
{
	...
	<tr>
		<td class="tdimg rl" style="text-align:center">
		<img src="'.GetRootPathForResources("images/layout_tabular.png").'" id="stepped" border="0" alt="'."Layout Tabular".'" />
		</td>
	</tr>
		';
