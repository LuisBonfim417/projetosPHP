...
if ( isset( $_POST['str_xml'] ) && isset( $_POST['web'] ) && !isset( $_POST['save'] ) )
{
	...
	{
		echo "Ao menos uma série deve ser selecionada";
		return;
	}
...
elseif ( isset( $_POST['del'] ))
{
	    if (count(GetUserGroups()) > 1)
	    {
	        ...
	        foreach ( $arr_reports as $rpt ) {
	            ...
	            {
					if($s=="report")
						echo "<p>"."Você não possui permissão para eliminar este relatório"."</p>";
					else
						echo "<p>"."Você não possui permissão para eliminar este gráfico"."</p>";
                	exit();
	            }
        	}
	    }
...
function Check_Crosstable_Group($arr,$is_save)
{
	...
	if($arr["group_fields"][count($arr["group_fields"])-1]["cross_table"]=="true")
	{
		...
		{
			echo "Você deve selecionar ao menos um grupo de campos";
			exit();
		}
		$count_x=0;
		$count_y=0;
		...
		{
			echo "Voçê deve selecionar ao menos um eixo";
			exit();
		}
	}
}
function Check_Crosstable_Totals($arr,$is_crosstable)
{
	if($is_crosstable=="true" && !empty($arr["totals"]))
	{
		...
		echo "Você deve verificar as funções de grupo";
		exit();
	}
}
?>